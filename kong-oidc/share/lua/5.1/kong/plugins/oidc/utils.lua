...
function M.get_options(config, ngx)
  return {
    ...
    groups_mapping_field = config.groups_mapping_field,
    groups_authorized_paths = config.groups_authorized_paths,
    -- Issue with the storage of the user into the lua-resty-oidc session so it's disable for now
    session_contents = {
      id_token=true,
      user=false,
      enc_id_token=true,
...
function M.isUserAuthorized(user_groups, groups_authorized_paths, requested_uri)
  if user_groups == nil or groups_authorized_paths == nil then
    return false
  end
  for key, group in pairs(user_groups) do
    for key, group_authorized_paths in pairs(groups_authorized_paths) do
      if group_authorized_paths.group_name == group then
        for key, authorized_path in pairs(group_authorized_paths.group_authorized_paths) do
          if string.sub(requested_uri, 1, #authorized_path) == authorized_path then
            return true
          end
        end
      end
...
-- transform a table into string, used for debug purpose
function dump(o)
